<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>ApplicationId</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            ApplicationNumber :  {{item.applicationNumber }} <br>
                            ServiceType :  {{item.serviceType }} <br>
                            ApplicationStatus :  {{item.applicationStatus }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <ApplicationId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import ApplicationId from '../ApplicationId.vue';

    export default {
        name: 'ApplicationIdManager',
        components: {
            ApplicationId
        },
        props: {
