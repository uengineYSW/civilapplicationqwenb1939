<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>DataCollectorId</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            DataCollectorId :  {{item.dataCollectorId }} <br>
                            CollectionStatus :  {{item.collectionStatus }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <DataCollectorId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import DataCollectorId from '../DataCollectorId.vue';

    export default {
        name: 'DataCollectorIdManager',
        components: {
            DataCollectorId
        },
        props: {
