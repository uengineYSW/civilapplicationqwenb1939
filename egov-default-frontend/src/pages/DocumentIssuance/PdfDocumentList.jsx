...
function EgovNoticeList(props) {
    ...
    const retrieveList = useCallback((searchCondition) => {
        ...
        const retrieveListURL = '/pdfDocuments';
        ...
        EgovNet.requestFetch(retrieveListURL,
            ...
            (resp) => {
                ...
                for(let i = 0; i< resp.length; i++){
                    ...
                    mutListTag.push(
                        <Link
                            to={{pathname: "/pdfDocument/PdfDocumentDetail"}}
                            state={{
                                documentId: resp[i].documentId,
                                searchCondition: searchCondition}}                            
                            className="list_item">
                            <div>{resp[i].documentId}</div>
                            <div>{resp[i].content}</div>
                            <div>{resp[i].generationDate}</div>
                            <div>{resp[i].status}</div>
                            <div>{resp[i].userData}</div>
                            <div>{resp[i].documents}</div>
                       </Link>
                    );
                };
                ...
                <div className="layout">
                    ...
                    <div className="contents NOTICE_LIST" id="contents">
                        ...
                        <div className="top_tit">
                            <h1 className="tit_1">문서 발급</h1>
                        </div>

                        <h2 className="tit_2">PDF문서</h2>
                        ...
                        <div className="condition">
                            <ul>
                                ...
                                <li className="third_2 R">
                                    ...
                                    <li>
                                        <Link to="/documentIssuance/PdfDocumentEdit" state={{bbsId: bbsId}} className="btn btn_blue_h46 pd35">등록</Link>
                                    </li>
                            </ul>
                        </div>
                        {/* <!--// 검색조건 --> */}
